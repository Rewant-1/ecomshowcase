      ...
      <div className="max-w-6xl mx-auto px-5 sm:px-6">
        <div className="flex items-center justify-between h-16 md:h-20">
          ...
          <div className="flex-shrink-0 mr-6">
            <Link to="/" className="block" aria-label="Stellar Precision">
          ...
          <nav className="flex flex-grow">
            <ul className="flex flex-grow justify-end flex-wrap items-center text-sm md:text-base">
              <li>
                <Link to="/categories" className="font-medium text-gray-600 hover:text-blue-600 px-3 py-2 transition">Categories</Link>
              </li>
              {categories.map(c => (
                <li key={c.slug} className="hidden lg:block">
                  <Link to={`/categories/${c.slug}`} className="text-gray-500 hover:text-blue-600 px-3 py-2 transition">{c.name}</Link>
                </li>
              ))}
            </ul>
          </nav>
